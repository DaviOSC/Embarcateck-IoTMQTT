...
static void update_fan_buzzer_output() {
    ...
    if (fan_on) {
        ...
        if (fan_speed == 0 && fan_on) target_frequency = 300;
        else if (fan_speed == 0) target_frequency = 0;
        ...
        if (target_frequency > 0) {
            ...
            if (wrap_value < 100) wrap_value = 100;
            if (wrap_value > 0xFFFF) wrap_value = 0xFFFF;

            pwm_set_wrap(buzzer_pwm_slice, wrap_value);
            pwm_set_gpio_level(BUZZER_PIN, wrap_value / 10); // 10% duty cycle
...
static void fan_joystick_adc_worker(async_context_t *context, async_at_time_worker_t *worker_task) {
    ...
    adc_select_input(1); // Seleciona ADC1 (joystick)
...
static void handle_incoming_mqtt_payload(void *arg, const u8_t *payload_data, u16_t payload_len, u8_t flags) {
    ...
    // Tópico já está em current_state->received_topic
    ...
    if (strcmp(current_state->received_topic, "/fan/control/set_state") == 0) {
        ...
        if (strncmp(current_state->received_payload, "ON", current_state->received_payload_len) == 0) new_fan_state = true;
        else if (strncmp(current_state->received_payload, "OFF", current_state->received_payload_len) == 0) new_fan_state = false;
        else { INFO_printf("Invalid payload for /fan/control/set_state.\n"); return; }

        if (fan_on != new_fan_state) { 
            fan_on = new_fan_state; 
            if (!fan_on) fan_speed = 0; // Se desligar, zera a velocidade
    ...
    else if (strcmp(current_state->received_topic, "/fan/control/set_speed") == 0)
    {
        ...
        if (new_speed_val < 0) new_speed_val = 0; 
        if (new_speed_val > 255) new_speed_val = 255;
    ...
    // Comando para ativar/desativar modo automático
        ...
        if (strncmp(current_state->received_payload, "ON", current_state->received_payload_len) == 0) new_auto_mode_val = true;
        else if (strncmp(current_state->received_payload, "OFF", current_state->received_payload_len) == 0) new_auto_mode_val = false;
        else { INFO_printf("Invalid payload for /fan/control/set_auto_mode.\n"); return; }

        if (auto_fan != new_auto_mode_val) {
            auto_fan = new_auto_mode_val;
            fan_settings_updated_by_command = true;
            if (auto_fan && fan_on) { // Se modo auto ativado e ventilador ligado, atualiza fan_speed
                adc_select_input(1); 
...
static void initiate_mqtt_connection(MQTT_CLIENT_STATE_T *current_state) {
    ...
    if (current_state->mqtt_client_inst == NULL) { // Cria instância apenas se não existir
